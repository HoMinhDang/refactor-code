...
# --- Các hàm khởi tạo model DeiT (Đã sửa để không yêu cầu file local) ---
...
def deit_tiny_patch16_256(pretrained=False, **kwargs):
    # Nếu pretrained=True, timm sẽ tự tải từ server nếu có, 
    # ở đây tôi chỉnh mặc định load từ kiến trúc để tránh lỗi file local
    ...
    if pretrained:
        try:
            ...
            print("Warning: Pretrained file not found. Initializing with random weights.")
...
# --- Các lớp phụ trợ (Giữ nguyên logic của bạn) ---

class resnet34(nn.Module):
    def __init__(self, pretrained=False): # Chỉnh mặc định thành False
        super().__init__()
        # weights=None tương đương pretrained=False trong các bản torchvision mới
        self.features = models.resnet34(weights=models.ResNet34_Weights.DEFAULT if pretrained else None)
    ...
    def forward(self, input):
        ...
        # SỬA TẠI ĐÂY: Tách ra từng dòng để biến được khởi tạo tuần tự
        f1 = self.layer1(x0)
        f2 = self.layer2(f1)
        f3 = self.layer3(f2)
        f4 = self.layer4(f3)
...
# ... (Các lớp PyramidPoolingModule, SEBlock, ResBlock, FeatureFusion, RPMBlock, DecoderBottleneckLayer giữ nguyên) ...
...
class DecoderBottleneckLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu3(x)
        return x
# --- CTCNet (Đã sửa lỗi khởi tạo FAM và tham số pretrained) ---

@register_model("dtrcnet")
class CTCNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        ...
        size = 256
        self.cnn = resnet34(pretrained=True) # Fix pretrain False
        self.headpool = PyramidPoolingModule()

        transformer = deit_tiny_distilled_patch16_256(pretrained=False) # Fix pretrain False
        ...
        # Khôi phục ModuleList dùng chung block (như yêu cầu "như ban đầu")
    ...
    def forward(self, x):
        ...
        # Khôi phục luồng forward thủ công của bạn
        emb = self.patch_embed(x)
        emb = self.transformers[0](emb)
        emb = self.transformers[1](emb)
        emb1 = self.transformers[2](emb)
        feature_tf1 = emb1.permute(0, 2, 1).view(b, size, 16, 16)

        emb1 = self.transformers[3](emb1)
        emb1 = self.transformers[4](emb1)
        emb2 = self.transformers[5](emb1)
        feature_tf2 = emb2.permute(0, 2, 1).view(b, size, 16, 16)

        emb2 = self.transformers[6](emb2)
        emb2 = self.transformers[7](emb2)
        emb3 = self.transformers[8](emb2)
        feature_tf3 = emb3.permute(0, 2, 1).view(b, size, 16, 16)

        emb3 = self.transformers[9](emb3)
        emb3 = self.transformers[10](emb3)
        emb4 = self.transformers[11](emb3)
        feature_tf = emb4.permute(0, 2, 1).view(b, size, 16, 16)

        feature_cat = self.fusion(feature_cnn, feature_tf)
        feature_cat = self.se(feature_cat)

        e1 = self.se1(self.fusion1(e1, feature_tf1))
        e2 = self.se2(self.fusion2(e2, feature_tf2))
        e3 = self.se3(self.fusion3(e3, feature_tf3))

        for i in range(2): e3 = self.FAM3[i](e3)
        for i in range(4): e2 = self.FAM2[i](e2)
        for i in range(6): e1 = self.FAM1[i](e1)

        d4 = self.decoder4(feature_cat) + e3
        d3 = self.decoder3(d4) + e2
        d2 = self.decoder2(d3) + e1
        
        out1 = self.final_relu1(self.final_conv1(d2))
        out = self.final_relu2(self.final_conv2(out1))
        out_0 = self.final_conv3(out)
        
        return out_0, torch.sigmoid(out_0)